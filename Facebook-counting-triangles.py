...
# Two triangles are considered to be the same if they can both be placed on the plane such that
...
class Solution:
    def countDistinctTriangles(self, arr):
        # congruent triangle - SSS
        unique = set()
        for a in arr:
            unique.add(tuple(sorted(a)))
        return len(unique)


s = Solution()
arr_1 = [(7, 6, 5), (5, 7, 6), (8, 2, 9), (2, 3, 4), (2, 4, 3)]
a = s.countDistinctTriangles(arr_1)
print(a)
arr_2 = [(3, 4, 5), (8, 8, 9), (7, 7, 7)]
a = s.countDistinctTriangles(arr_2)
print(a)
