...
# which may be obtained from it after performing at most k element swaps,
...
class Solution:
    def findMinArray(self, arr, k):
        # try to bubble up min element
        start = 0
        while start < len(arr) - 1 and k > 0:
            min_val = arr[start]
            min_i = start
            if start + k + 1 <= len(arr):
                bound = start + k + 1
            else:
                bound = len(arr)
            # find min val
            ...
            if min_i > start:
                ...
                for _ in range(swaps):
                    arr[min_i - 1], arr[min_i] = arr[min_i], arr[min_i - 1]
                    min_i -= 1
