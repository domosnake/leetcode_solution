...
# shuffles the characters in such a way that the resulting string is hard to quickly read,
...
# and then append the encrypted version of "a" (which is just "a") and
# the encrypted version of "c" (which is just "c") to get "bac".
...
class Solution:
    def findEncryptedWord(self, s):
        encrypted = ['']
        self._helper(s, encrypted)
        return encrypted[0]

    def _helper(self, s, encrypted):
        if not s:
            return
        ...
        if len(s) % 2 == 0:
            ...
            mid = len(s) // 2
        encrypted[0] += s[mid]
        self._helper(s[:mid], encrypted)
        self._helper(s[mid + 1:], encrypted)


s = Solution()
# bacd
a = s.findEncryptedWord('abcd')
print(a)
